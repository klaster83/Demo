...
class GrowattApi:
    ...
    def plant_detail(self, plant_id, timespan, date_str):
        """
        ...
        * Timespan.total: power on each year. `date` parameter is ignored.
        ...
        response = self.session.get(
            ...
            params={"plantId": plant_id, "type": timespan, "date": date_str},
        )
        return self._back_success_response(response)

    def new_plant_detail(self, plant_id, timespan, date_str):
        """
        Return amount of power generated for the given timespan.
...
response1 = session.login("", "")

print(response1)
plant_ID = response1["data"][0]["plantId"]

response2 = session.new_plant_detail(plant_ID, 1, "2022-06-14")
my_dict = response2["data"]
# for _ in response2["data"]:
#     print(_)

print(sorted(my_dict)[0], sorted(my_dict)[-1])
